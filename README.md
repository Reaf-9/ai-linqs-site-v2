...
v1（`ai-linqs-site-pages` コミット `759e474`）の7ページの文言を、すべてこの1ページに移植している。2026-09-19 に「横文字を減らして分かりやすく」の指示で、下記だけ言い換えた（事実・数字は変えていない）。

### v1 からの文言変更（2026-09-19）

| v1 | v2 |
|---|---|
| ROI起点で話す | 投資に見合うかで話す |
| 3ヶ月のコーチング／コーチングは3ヶ月を基本に | 3ヶ月の伴走研修／研修は3ヶ月を基本に |
| ①コーチング事業 | ①研修・伴走事業 |
| 広告内製化／LPコーチング（Meta・Google広告やLPを…） | 広告・集客ページの内製化支援（…集客ページ（LP）を…） |
| ルール・テンプレート | ルール・ひな形 |
| ポータルアプリ | 社内で共有・更新できるアプリ |
| 判断のボトルネック | 判断の詰まり |
| AIリテラシーが向上 | AIを使いこなす力が向上 |
| リスクマネジメント | リスク管理 |
| 教育コスト | 教育の費用 |
| 社内DX整備 | 社内のデジタル環境整備 |
| プレミアム（AI顧問のタグ） | 継続支援 |
| 「共に」が、AI Linqsのアイデンティティ | 「共に」が、AI Linqsらしさ |
| Before／After | 導入前／導入後 |
| 現場をヒアリングし | 現場の話をお聞きし |
| サービスラインナップ | サービス一覧 |
| ナビ・見出しの英語ラベル（Service／Works／Issues／Mission ほか） | 日本語（サービス／実績／課題／使命 ─ Mission ほか） |

図解（`.fig`）は HTML＋CSS の箱と矢印だけで作っている（外部ライブラリなし・数字は入れない）。比較表は小窓からスライド `#compare` に移動。
...
| `favicon-16/32/180/192/512.png` | LP 用に作ったロゴ画像 `03_LP/…/logo-ailinqs-white.png` から「AI」部分を切り抜き、サイトの青（#8ecbff）に着色して紺の角丸に載せたもの。正式ロゴが届いたら作り直す |
...
- [x] 横文字の言い換え（2026-09-19、山本さん指示）。変更一覧は下記「v1 からの文言変更」
